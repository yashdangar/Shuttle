...
router.get(
  ...
  (req, res) => {
    ...
    const token = jwt.sign(
      { userId: user.id, email: user.email, hotelId: user.hotelId || null, role: 'guest' },
    ...
    if (req.query.state || user.hotelId) {
      ...
      res.redirect(
        `${process.env.GUEST_FRONTEND_URL}/auth/callback?token=${token}&hotelId=${hotelId}`
      );
    } else {
      // Redirect to frontend with token
      res.redirect(
        `${process.env.GUEST_FRONTEND_URL}/auth/callback?token=${token}`
      );
    }
  }
);
