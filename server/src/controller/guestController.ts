...
const getHotels = async (req: Request, res: Response) => {
  try {
    ...
    const hotelsWithAddressesAndImages = await Promise.all(
      hotels.map(async (hotel) => {
        ...
        if (hotel.imagePath) {
          try {
            imageUrl = await getSignedUrlFromPath(hotel.imagePath);
          } catch (imageError) {
            console.error(
              `Error getting image URL for hotel ${hotel.id}:`,
              imageError
            );
          }
        }
    ...
    res.json({ hotels: hotelsWithAddressesAndImages });
  } catch (error) {
...
const getLocations = async (req: Request, res: Response) => {
  try {
    ...
    const locations = hotelLocations.map((hl) => ({
      ...
      price: hl.price, // Include pricing information
    }));
    res.json({ locations });
  } catch (error) {
...
const createTrip = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (pickupLocationId || dropoffLocationId) {
      // Determine which location to use for pricing based on trip type
    ...
    const trip = await prisma.booking.create({
      data: {
        ...
        // Pricing fields for performance optimization
        pricePerPerson,
        totalPrice,
        createdAt: new Date(),
        updatedAt: new Date(),
  ...
  try {
    ...
    const tripsWithETA = await Promise.all(
      trips.map(async (trip) => {
        ...
        let pricePerPerson = trip.pricePerPerson || 0;
        let totalPrice = trip.totalPrice || 0;
          ...
          if (destination) {
            try {
              ...
              if (trip.tripId) {
                ...
                etaResult = await googleMapsService.calculateETA(
                  origin,
                  destination
                );
              } else {
  ...
  try {
    ...
    const bookingsWithETA = await Promise.all(
      currentBookings.map(async (currentBooking) => {
        ...
        // Use stored pricing
        pricePerPerson = currentBooking.pricePerPerson || 0;
        totalPrice = currentBooking.totalPrice || 0;
...
const getCurrentBookingsByHotel = async (req: Request, res: Response) => {
  try {
    const guestId = (req as any).user.userId;
    ...
    // Get all location IDs associated with this hotel
    const hotelLocations = await prisma.hotelLocation.findMany({
      where: { hotelId },
      select: { locationId: true },
    });

    const locationIds = hotelLocations.map((hl) => hl.locationId);

    if (locationIds.length === 0) {
      return res.json({ currentBookings: [] });
    }

    // Get all active bookings for the guest that are associated with this hotel's locations
    const currentBookings = await prisma.booking.findMany({
      where: {
        guestId: guestId,
        isCompleted: false, // Only get active bookings
        isCancelled: false, // Exclude cancelled bookings
        OR: [
          { pickupLocationId: { in: locationIds } },
          { dropoffLocationId: { in: locationIds } },
        ],
      },
      include: {
        pickupLocation: true,
    ...
    console.error("Get current bookings by hotel error:", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
...
const getChats = async (req: Request, res: Response) => {
  ...
  try {
    // Verify the guest is associated with this hotel
    const guest = await prisma.guest.findUnique({
      where: { id: userId },
      select: { hotelId: true },
    });

    if (!guest || guest.hotelId !== parseInt(hotelId)) {
      return res.status(403).json({ error: "Access denied" });
    }

    // Get all frontdesk staff for this hotel
    ...
    // Get existing chats for this guest in the specified hotel
    ...
    // Create a map of existing chats by frontdesk ID
    ...
    // Create default chats with frontdesk staff if they don't exist
    const chatsToCreate = [];
    for (const frontdesk of frontdeskStaff) {
      if (!existingChatsMap.has(frontdesk.id)) {
        chatsToCreate.push({
          guestId: userId,
    ...
    // Create the missing chats in bulk
    ...
    // Fetch all chats (including newly created ones) with proper includes
...
const getChatMessages = async (req: Request, res: Response) => {
  ...
  try {
    // Verify the chat belongs to this guest and hotel
