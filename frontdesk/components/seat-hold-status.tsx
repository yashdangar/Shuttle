...
import { Clock, AlertCircle, CheckCircle, RefreshCw, Users } from 'lucide-react';
...
export default function SeatHoldStatus({ booking, onRefresh, className = "" }: SeatHoldStatusProps) {
  ...
  const handleConfirmSeats = async () => {
    ...
    try {
      setIsLoading(true);
      await api.post(`/frontdesk/bookings/${booking.id}/verify`, {});
      toast.success('Seats confirmed successfully');
      onRefresh?.();
    } catch (error: any) {
      console.error('Error confirming seats:', error);
      toast.error(error.message || 'Failed to confirm seats');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="space-y-3">
            ...
            {booking.shuttle && (
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-medium">{booking.shuttle.vehicleNumber}</span>
              </div>
            )}
            ...
            {isExpired ? (
              <div className="bg-red-100 border border-red-200 rounded p-2">
                ...
                <p className="text-xs text-red-700 mt-1">
                  The seat hold has expired. You may need to reassign this booking.
                </p>
              </div>
            ) : (
              <div className="bg-blue-100 border border-blue-200 rounded p-2">
                ...
                <p className="text-xs text-blue-700 mt-1">
                  This booking is ready for frontdesk verification to confirm the seats.
                </p>
                <Button
                  size="sm"
                ...
                >
                  {isLoading ? 'Confirming...' : 'Confirm Seats'}
                </Button>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-green-100 border border-green-200 rounded p-2">
              ...
              <p className="text-xs text-green-700 mt-1">
                This booking has been verified and seats are confirmed. The guest can now use their QR code.
              </p>
            </div>
          </div>
        </CardContent>
