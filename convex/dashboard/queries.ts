...
export const getDashboardStats = query({
  ...
  async handler(ctx, args) {
    ...
    const completedTrips = hotelTripInstanceIds.filter(
      (t) => t.status === "COMPLETED"
    ).length;
    const inProgressTrips = hotelTripInstanceIds.filter(
      (t) => t.status === "IN_PROGRESS"
    ).length;
    const scheduledTrips = hotelTripInstanceIds.filter(
      (t) => t.status === "SCHEDULED"
    ).length;

    // Today's trips
    const todayTrips = hotelTripInstanceIds.filter(
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      dailyBookings.push({
        ...
        confirmed: dayBookings.filter((b) => b.bookingStatus === "CONFIRMED")
          .length,
        pending: dayBookings.filter((b) => b.bookingStatus === "PENDING")
          .length,
        rejected: dayBookings.filter((b) => b.bookingStatus === "REJECTED")
          .length,
    ...
    const paymentStats = {
      ...
      refunded: allBookings.filter((b) => b.paymentStatus === "REFUNDED")
        .length,
    ...
    const paymentMethodStats = {
      ...
      frontdesk: allBookings.filter((b) => b.paymentMethod === "FRONTDESK")
        .length,
...
export const getFrontdeskDashboardStats = query({
  args: {
    frontdeskId: v.id("users"),
  },
  async handler(ctx, args) {
    const frontdesk = await ctx.db.get(args.frontdeskId);
    if (!frontdesk || frontdesk.role !== "frontdesk" || !frontdesk.hotelId) {
      return null;
    }

    const hotelId = frontdesk.hotelId;
    const today = getUTCDateString();
    const thirtyDaysAgo = getDateStringDaysAgo(30);
    const sevenDaysAgo = getDateStringDaysAgo(7);
    ...
    // Daily booking data for the last 30 days (without earnings)
    const dailyBookings = [];
    for (let i = 29; i >= 0; i--) {
      const date = getDateStringDaysAgo(i);
      const dayBookings = allBookings.filter((b) => {
...
export const getFrontdeskRecentBookings = query({
  args: {
    frontdeskId: v.id("users"),
    limit: v.optional(v.number()),
  },
  async handler(ctx, args) {
    ...
    const allBookings = await ctx.db
      .query("bookings")
      .withIndex("by_hotel", (q) => q.eq("hotelId", frontdesk.hotelId!))
      .collect();
...
export const getFrontdeskActiveTrips = query({
  args: {
    frontdeskId: v.id("users"),
  },
  async handler(ctx, args) {
