...
export const updateAdmin = action({
  ...
  async handler(ctx, args): Promise<AdminAccount> {
    ...
    const existing = await ctx.runQuery(api.admins.getAdminById, {
      userId: args.userId,
    });
    ...
    await ctx.runMutation(internal.admins.updateAdminInternal, {
      userId: args.userId,
      ...payload,
    });

    const updated = await ctx.runQuery(api.admins.getAdminById, {
      userId: args.userId,
    });
...
export const deleteAdmin = action({
  ...
  async handler(ctx, args): Promise<{ success: true }> {
    ...
    await ctx.runMutation(internal.admins.deleteAdminInternal, {
      userId: args.userId,
    });
